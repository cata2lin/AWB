...
# Add useCallback, memo to react imports
content = re.sub(r"import \{ useState, useEffect, useMemo, Fragment \} from 'react'", 
                 "import { useState, useEffect, useMemo, Fragment, useCallback, memo } from 'react'", content)

velocity_row_code = """
// --- Memoized Row for Sales Velocity to fix checkbox lag ---
...
const VelocityRow = memo(({ 
    p, i, rowKey, isSelected, isExpanded, velocityMetricsType, velocityTargetCoverage, 
    visCols, velocityViewMode, onToggleSelect, onToggleExpand, setSelectedVariantOrders 
}) => {
    const isColVisible = (col) => visCols.has(col) || velocityViewMode === 'all';
                                    ...
                                    <div className="space-y-1 text-xs">
                                        {p.by_country.map(bc => (
                                            <div key={bc.country} className="flex justify-between">
                                                <span className="text-zinc-600 dark:text-zinc-400">{bc.country}</span>
...
content = content.replace("export default function Analytics() {", velocity_row_code)

# Add callbacks to Analytics()
callbacks = """
...
"""
content = content.replace("const [velocityExpanded, setVelocityExpanded] = useState(null)", "const [velocityExpanded, setVelocityExpanded] = useState(null)\n" + callbacks)

# Replace the map block
old_block_start = "                                                                    {sortedProducts.map((p, i) => {"
old_block_end = "                                                                    })}"

# Find the block and replace it
start_idx = content.find(old_block_start)

# Finding the end of the map block is tricky, we can use regex or string manipulation
if start_idx != -1:
    ...
    new_block = """                                                                    {sortedProducts.map((p, i) => {
                                                                        ...
                                                                        return (
                                                                            <VelocityRow 
                                                                                ...
                                                                                visCols={visCols}
                                                                                velocityViewMode={velocityViewMode}
