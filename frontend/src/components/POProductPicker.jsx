 ...
 *
 * The full catalogue + analytics + store list are cached at module scope so
 * opening the picker a second time is instant. A manual refresh button
 * (top-right) busts the cache when stock-sync freshness matters.
 */
...
// Module-level cache shared across modal open/close cycles.
// `catalogue` is { products, stock_synced_at } from the productPicker API.
// `analytics` is { sku: row } from the purchase-orders analytics API.
// `stores` is the raw store list from storesApi.
...
export default function POProductPicker({ existingSkus = [], onAddProducts, onClose, categories = [] }) {
  ...
  // ── Load full catalogue ONCE on mount, with module-level cache ──
  const loadCatalogue = useCallback(async (force = false) => {
    if (!force && _catalogueCache) {
      setAllProducts(_catalogueCache.products || [])
      setStockSyncedAt(_catalogueCache.stock_synced_at || null)
      setCatalogueLoading(false)
      return
    }
    setCatalogueLoading(true)
    try {
      const r = await purchaseOrdersMgmtApi.productPicker({ limit: 1000 })
      _catalogueCache = { products: r.products || [], stock_synced_at: r.stock_synced_at || null }
      setAllProducts(_catalogueCache.products)
      setStockSyncedAt(_catalogueCache.stock_synced_at)
    } catch {
      // Stay silent — input still works without catalogue (empty list).
    } finally {
      setCatalogueLoading(false)
    }
  }, [])
  ...
  const loadStores = useCallback(async (force = false) => {
    if (!force && _storesCache) {
      setAllStores(_storesCache)
      return
    }
    try {
      const r = await storesApi.getStores()
      _storesCache = (r.stores || r || []).filter(s => s.name)
      setAllStores(_storesCache)
    } catch { /* silent */ }
  }, [])

  const loadAnalytics = useCallback(async (force = false) => {
    if (!force && _analyticsCache) {
      setAnalyticsData(_analyticsCache)
      setAnalyticsLoading(false)
      return
    }
    setAnalyticsLoading(true)
    try {
      const r = await analyticsApi.getPurchaseOrders({ days: 30 })
      const map = {}
  ...
  }, [loadCatalogue, loadStores, loadAnalytics])
  ...
  const refreshAll = () => {
    ...
    loadStores(true)
    loadAnalytics(true)
    toastSuccess('Catalog reîncărcat')
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
      ...
      <div className="relative w-[95vw] max-w-5xl h-[85vh] bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-700 flex flex-col overflow-hidden animate-in">
        ...
        <div className="px-5 pt-4 pb-3 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/80 space-y-3">
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input ref={inputRef} value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full pl-10 pr-10 py-2.5 text-sm rounded-xl border border-zinc-200 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white placeholder-zinc-400 focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 transition-all" />
            ...
            <div className="relative flex-shrink-0">
              <select value={sortBy} onChange={e => setSortBy(e.target.value)}
                className="pl-8 pr-3 py-2 text-xs rounded-xl border border-zinc-200 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 focus:ring-2 focus:ring-primary-500/30 appearance-none cursor-pointer">
            ...
            {catalogueLoading && <RefreshCw className="w-4 h-4 text-primary-400 animate-spin flex-shrink-0" />}
            <button onClick={refreshAll} title="Reîncarcă catalogul (forțează refetch de la backend)"
              className="p-2 text-zinc-400 hover:text-primary-600 dark:hover:text-primary-400 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-700 flex-shrink-0"
              disabled={catalogueLoading}>
              <RefreshCw className={`w-4 h-4 ${catalogueLoading ? 'animate-spin' : ''}`} />
            </button>
          ...
          {/* Stock freshness — read by the backend stock_sync_service every 15 min */}
          {stockSyncedAt && (() => {
            const synced = new Date(stockSyncedAt)
            const ageMs = Date.now() - synced.getTime()
            const ageMin = Math.round(ageMs / 60000)
            const tone = ageMin > 30
              ? 'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-500/30'
              : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-500/30'
            const label = ageMin < 1 ? 'acum câteva secunde' : ageMin === 1 ? 'acum 1 minut' : `acum ${ageMin} min`
            return (
              <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[11px] font-medium w-fit ${tone}`}
                title={`Stoc citit din InventorySync la ${synced.toLocaleString('ro-RO')}`}>
                <span className="w-1.5 h-1.5 rounded-full bg-current" />
                Stoc actualizat {label}
              </div>
            )
          })()}
          ...
          {categories.length > 0 && (
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <button onClick={() => switchCategory('')}
                className={`px-2.5 py-1 text-[11px] font-medium rounded-lg transition-all ${!activeCategory ? 'bg-primary-100 dark:bg-primary-500/20 text-primary-700 dark:text-primary-300' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400'}`}>
                All Products
              </button>
              {categories.map(c => (
                <button key={c.key} onClick={() => switchCategory(c.key)}
                  className={`px-2.5 py-1 text-[11px] font-medium rounded-lg transition-all ${activeCategory === c.key ? 'bg-primary-100 dark:bg-primary-500/20 text-primary-700 dark:text-primary-300' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400'}`}>
                  {c.label}
                </button>
              ))}
              {/* Store dropdown */}
                ...
                {showStoreDropdown && (
                  <div className="absolute top-full left-0 mt-1 w-56 max-h-60 overflow-y-auto bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-600 z-50 py-1" onClick={e => e.stopPropagation()}>
                    {allStores.map(s => {
                      ...
                      return (
                        <label key={s.uid || s.name} className={`flex items-center gap-2 px-3 py-1.5 text-xs cursor-pointer ${isInCat ? 'bg-primary-50 dark:bg-primary-500/10' : 'hover:bg-zinc-50 dark:hover:bg-zinc-700/50'}`}>
                          ...
                          <span className={`truncate ${isInCat ? 'text-primary-600 dark:text-primary-400' : 'text-zinc-700 dark:text-zinc-300'}`}>{s.name}</span>
                          {isInCat && <span className="ml-auto text-[9px] text-primary-400">category</span>}
                        </label>
                      )
                    })}
                  </div>
        ...
        {(results.length > 0 || catalogueLoading) && (
          <div className="flex items-center justify-between px-5 py-2 bg-zinc-50/50 dark:bg-zinc-800/30 border-b border-zinc-100 dark:border-zinc-700/50">
            <label className="flex items-center gap-2 cursor-pointer text-xs text-zinc-600 dark:text-zinc-400">
              <input type="checkbox" checked={selected.size > 0 && selected.size >= selectableResults.length} onChange={toggleAll} className="w-3.5 h-3.5 rounded border-zinc-300 text-primary-500" />
                ...
                <button key={p.uid || p.sku} onClick={() => !isAdded && toggleSelect(p.sku)} disabled={isAdded}
                  className={`relative group text-left rounded-xl border-2 transition-all duration-150
                    ${isAdded ? 'opacity-40 cursor-default border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800'
                      : isChecked ? 'border-primary-500 bg-primary-50 dark:bg-primary-500/10 shadow-md shadow-primary-500/10 scale-[1.02]'
                      : 'border-zinc-200 dark:border-zinc-700 hover:border-primary-300 dark:hover:border-primary-500/40 hover:shadow-md bg-white dark:bg-zinc-800'}`}>

                  {/* Checkbox */}
                  <div className={`absolute top-2 right-2 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all
                    ${isAdded ? 'border-green-400 bg-green-100 dark:bg-green-500/20' : isChecked ? 'border-primary-500 bg-primary-500' : 'border-zinc-300 dark:border-zinc-600 group-hover:border-primary-400'}`}>
                    {isAdded && <Check className="w-3 h-3 text-green-600 dark:text-green-400" />}
                    {isChecked && !isAdded && <Check className="w-3 h-3 text-white" />}
                  </div>

                  {/* Image — fall back to placeholder if the image URL 404s */}
                  <div className="px-3 pt-3 pb-1">
                    <div className="w-full aspect-square rounded-lg bg-zinc-100 dark:bg-zinc-700 overflow-hidden mb-2 flex items-center justify-center relative">
                      {p.image
                        ? <img
                            src={p.image}
                          ...
                          />
                        : null}
                      {/* Placeholder shown when no image OR when img onError fires */}
                      <Package className={`w-8 h-8 text-zinc-300 dark:text-zinc-600 ${p.image ? 'absolute' : ''}`} hidden={!!p.image} />
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      ...
                      <button onClick={e => { e.stopPropagation(); setEditingVariantUid(p.uid); setVariantForm({ v1: p.tom_variant_1 || '', v2: p.tom_variant_2 || '' }) }}
                        className="p-1 text-zinc-400 hover:text-primary-600 dark:hover:text-primary-400 z-10"><Edit2 className="w-3 h-3" /></button>
                    </div>
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-white dark:bg-zinc-800 p-3 shadow-lg border-t border-zinc-200 dark:border-zinc-700 z-20 rounded-b-xl" onClick={e => e.stopPropagation()}>
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2 pt-1">
                            ...
                            <button onClick={() => handleSaveVariants(p.uid)} className="flex-1 px-2 py-1 text-[10px] font-bold bg-primary-600 text-white rounded hover:bg-primary-700">Save</button>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="px-3 pb-3 pt-1.5 mt-1 border-t border-zinc-100 dark:border-zinc-700/50">
                    <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                      ...
                      <div>
                        ...
                        <p className={`text-xs font-bold ${a.suggested_qty > 0 ? 'text-primary-600 dark:text-primary-400' : 'text-zinc-400'}`}>{a.suggested_qty > 0 ? `+${fmt(a.suggested_qty)}` : '—'}</p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/80">
          <div className="text-xs text-zinc-500">
            {selected.size > 0
              ? <span className="text-primary-600 dark:text-primary-400 font-semibold">{selected.size} product{selected.size !== 1 ? 's' : ''} selected</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleAdd} disabled={selected.size === 0}
              className="flex items-center gap-2 px-5 py-2 text-sm font-bold bg-primary-600 hover:bg-primary-700 text-white rounded-lg disabled:opacity-30 disabled:cursor-not-allowed transition-all ">
