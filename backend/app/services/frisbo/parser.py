...
def parse_order(raw_order: Dict) -> Dict:
    ...
    active_items = [
        item for item in line_items if int(item.get("quantity", 0) or 0) > 0
    ]
    parsed["line_items"] = active_items
    parsed["item_count"] = (
        sum(int(item.get("quantity", 1)) for item in active_items)
        if active_items
        else 0
    )
    # SKU can be at top level or nested in inventory_item
    parsed["unique_sku_count"] = (
        len(set(_get_item_sku(item) for item in active_items if _get_item_sku(item)))
        if active_items
        else 0
    )
    ...
    for shipment in shipments_raw:
        ...
        latest_event = (
            events_obj.get("latest_event", {}) or {}
            if isinstance(events_obj, dict)
            else {}
        )
        processed_events = (
            events_obj.get("processed", []) or []
            if isinstance(events_obj, dict)
            else []
        )

        latest_event_key = (
            latest_event.get("key") if isinstance(latest_event, dict) else None
        )
        latest_event_date = (
            _parse_datetime(latest_event.get("date"))
        ...
        cod_value = (
            payment.get("cash_on_delivery_value") if isinstance(payment, dict) else None
        )
        cod_currency = payment.get("currency") if isinstance(payment, dict) else None
        is_cod = (
            payment.get("cash_on_delivery", False)
    ...
    # Full single-order GET has `note` (free text); search has `notes.selling_channel`.
    note = raw_order.get("note")
    if not note:
        notes_obj = raw_order.get("notes", {}) or {}
        if isinstance(notes_obj, dict):
            note = notes_obj.get("selling_channel")
    parsed["note"] = (note or "").strip() or None
