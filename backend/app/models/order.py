...
class Order(Base):
    ...
    store_uid: Mapped[str] = mapped_column(
        String(100), ForeignKey("stores.uid"), index=True
    )
    ...
    # Storefront tags (Frisbo OrderTags.selling_channel) — list of lowercased tag
    # keys, e.g. ["test", "duplicata"]. Powers test-order exclusion in reports.
    tags: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    # Order note (free-text / selling-channel note from Frisbo).
    ...
    awb_pdf_url: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # From shipments[]
    ...
    )  # e.g. "Plată ramburs" (COD), "Shopify Payments" (card)
    shipment_status: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True
    )  # not_created, created_awb, etc.
    aggregated_status: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True
    )  # not_fulfilled, waiting_for_courier, etc.
    is_printed: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    print_hold: Mapped[bool] = mapped_column(
        Boolean, default=False, index=True
    )  # Held due to duplicate detection

    # Multi-AWB support (max 10 per order)
    awb_count: Mapped[int] = mapped_column(
        Integer, default=1
    )  # Number of AWB labels (1-10)
    awb_count_manual: Mapped[bool] = mapped_column(
    ...
    shipping_data_source: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # csv_import | historical_match | manual
    shipping_data_manual: Mapped[bool] = mapped_column(
        Boolean, default=False
    )  # True = don't overwrite on import
    ...
    currency: Mapped[Optional[str]] = mapped_column(
        String(10), nullable=True, default="RON"
    )

    # Timestamps
    frisbo_created_at: Mapped[Optional[datetime]] = mapped_column(
    ...
    waiting_for_courier_since: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )  # Set when mark_waiting_for_courier called

    # Relationships
    store: Mapped["Store"] = relationship(back_populates="orders")
    batch_items: Mapped[List["PrintBatchItem"]] = relationship(back_populates="order")
    awbs: Mapped[List["OrderAwb"]] = relationship(
