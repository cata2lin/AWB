...
class ProfitabilityConfig(Base):
    ...
    agency_commission_excluded_stores: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, default=list
    )
    # Order tags excluded from agency commission (e.g., ["test"])
    agency_commission_excluded_tags: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, default=list
    )
    ...
    gt_commission_store_uid: Mapped[Optional[str]] = mapped_column(
    ...
    # VAT rate (default: 21% = 0.21). Used as the RO post-2025-08-01 rate and the
    # fallback when a store has no resolvable country. Per-country rates live in
    # app.core.vat.DEFAULT_VAT_RATES (PL 23%, BG 20%, CZ 21%).
    ...
    subscriptions: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, default=dict
    )

    # Monthly marketing costs (JSON: { "2026-02": { "facebook": 500, "google": 300, "tiktok": 200 } })
    marketing_costs: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, default=dict
    )
