...
async def get_deliverability_stats(
    ...
    ]  # drop excluded-tag orders: built-in test/sample + configured rules
    if store_uid_list:
    ...
    query = select(
        ...
        func.sum(
            case((Order.aggregated_status.in_(sorted(DELIVERED)), 1), else_=0)
        ).label("delivered"),
        func.sum(
            case((Order.aggregated_status.in_(sorted(CANCELLED)), 1), else_=0)
        ).label("cancelled"),
        func.sum(
            case((Order.aggregated_status.in_(sorted(RETURNED)), 1), else_=0)
        ).label("returned"),
        func.sum(
            case((Order.aggregated_status.in_(sorted(IN_TRANSIT)), 1), else_=0)
        ).label("in_transit"),
        func.sum(
            case((Order.aggregated_status.in_(sorted(OUT_FOR_DELIVERY)), 1), else_=0)
        ).label("out_for_delivery"),
        func.sum(
            case(
                (Order.aggregated_status.in_(["processing", "not_fulfilled"]), 1),
                else_=0,
            )
        ).label("processing"),
        func.sum(
            case((Order.aggregated_status == "ready_for_pickup", 1), else_=0)
        ).label("ready_for_pickup"),
        func.sum(case((Order.aggregated_status == "new", 1), else_=0)).label("new"),
        func.sum(
            case((Order.aggregated_status.in_(sorted(REFUSED)), 1), else_=0)
        ).label("refused"),
        func.sum(
            case((Order.aggregated_status == "waiting_for_courier", 1), else_=0)
        ).label("waiting_for_courier"),
    ...
    for row in rows:
        ...
        in_transit_rate = (
            ((in_transit + out_for_delivery) / shipped * 100) if shipped > 0 else 0
        )
    ...
    totals["in_transit_rate"] = round(
        ((totals["in_transit"] + totals["out_for_delivery"]) / shipped_total * 100)
        if shipped_total > 0
        else 0,
        2,
    )
    ...
    return {"stores": store_stats, "totals": totals}
