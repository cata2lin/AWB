...
async def get_deliverability_stats(
    ...
    query = select(
        ...
        func.sum(case((Order.aggregated_status.in_(['delivered', 'customer_pickup', 'in_parcel_locker']), 1), else_=0)).label('delivered'),
        func.sum(case((Order.aggregated_status == 'cancelled', 1), else_=0)).label('cancelled'),
        func.sum(case((Order.aggregated_status.in_(['back_to_sender', 'returning_to_sender', 'incorrect_address', 'lost']), 1), else_=0)).label('returned'),
        func.sum(case((Order.aggregated_status.in_(['in_transit', 'fulfilled', 'redirected', 'deferred_delivery', 'on_hold']), 1), else_=0)).label('in_transit'),
        func.sum(case((Order.aggregated_status == 'out_for_delivery', 1), else_=0)).label('out_for_delivery'),
        func.sum(case((Order.aggregated_status.in_(['processing', 'not_fulfilled']), 1), else_=0)).label('processing'),
