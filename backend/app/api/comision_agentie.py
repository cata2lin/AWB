...
def _month_range(month: str):
    ...
    last_day = (dt_date(end_year, end_mon, 1) - __import__('datetime').timedelta(days=1)).day
...
SHIPPED_STATUSES = {
    ...
    'customer_pickup', 'in_parcel_locker', 'fulfilled',
    'waiting_for_courier', 'picked_up', 'redirected', 'deferred_delivery',
    'returning_to_sender', 'back_to_sender', 'returned',
    'unsuccessful_delivery', 'refused', 'lost',
}
...
async def _calculate_comision(
    ...
    if store_comision_pct:
        pct_overrides = ",".join(f"{k}:{v}" for k, v in sorted(store_comision_pct.items()))
    cache_key = f"{month}_{comision_pct}_{store_uids_key}_{pct_overrides}_v3"
    ...
    for s_pnl in prof_data.get('pnl_by_store', []):
        ...
        # Use sales_delivered for exact match with Profitability tab's "Realized Revenue"
        incasari = s_pnl['income']['sales_delivered']['cu_tva']
    ...
    formula_str = f"Procent Variabil × (Încasări Realizate - Transport)" if has_overrides else f"{comision_pct}% × (Încasări Realizate - Transport)"
...
async def get_comision_config(db: AsyncSession = Depends(get_db)):
    ...
    stores_result = await db.execute(
        select(Store.uid, Store.name).order_by(Store.name)
    )
...
async def export_comision_csv(
    ...
    writer.writerow([
        "LUNA", "BRAND", "COST", "COMENZI", "PLECATE", "REFUZATE",
        "ÎNCASĂRI", "COMISION %", "TRANSPORT", "TOTAL COMISION"
    ])
    for row in data["stores"]:
        writer.writerow([
            row["luna"], row["brand"], row["cost"],
            row["comenzi"], row["plecate"], row["refuzate"],
            row["incasari"], row["comision_pct"],
            row["transport"], row["total_comision"],
        ])
